...
def train(data, fdr_cutoff=0.05, sample_frac=1, sample_state=42):
    ...
    psms_train = data.psms[(data.psms["FDR"] <= fdr_cutoff) & (data.psms["isTT"])]
    psms_train = psms_train.sample(frac=sample_frac, random_state=sample_state)
    psms_train = psms_train[psms_train["Duplicate"] == False]
    psms_train["depart_training"] = True
    psms_train = psms_train[psms_train.Duplicate == False]

    # psms not involved in training
    psms_nontrain = data.psms.loc[psms_df.index.difference(psms_train.index)]
    ...
    y_train = xirtnet.prepare_multitask_y(psms_train.loc[psms_train.index],
                                          xiRT_params["output"])
