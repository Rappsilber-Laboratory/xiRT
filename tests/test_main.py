...
def test_xirt_runner_rp_crosslinks_cv(tmpdir):
    ...
    __main__.xirt_runner(peptides_file=peptides_in, out_dir=tmpdir.mkdir("xiRT_results"),
                         xirt_params=xirt_params, learning_params=learning_params, nrows=500, perform_qc=False,
                         write_dummy=False)
    assert True
...
def test_xirt_runner_scx_crosslinks_cv_refit(tmpdir):
    ...
    with open(setup_loc, "r") as file:
        learning_params=yaml.safe_load(file)
    __main__.xirt_runner(peptides_file=peptides_in, out_dir=tmpdir.mkdir("xiRT_results"),
                         xirt_params=xirt_params, learning_params=learning_params, nrows=1000, perform_qc=False,
                         write_dummy=False)
    assert True
